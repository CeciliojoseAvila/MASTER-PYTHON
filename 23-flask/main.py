...
#clave secreta para poder crear coche  con mensaje flash
app.secret_key = 'clave_secreta_flask'

# Conexion a la DB
app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PASSWORD'] = '8629'
app.config['MYSQL_DB'] = 'proyectodflask'
...
def index():
    ...
    edad = 85
    personas = ['Cecilio', 'Victor', 'Paco', 'Francisco', 'David']

    return render_template('index.html', edad=edad, dato1="valor1",
                           dato2="valor2",
                           lista=["uno", "dos", "tres"],
                           personas=personas
                           )
...
def informacion(nombre=None, apellidos=None):
    ...
    if nombre != None and apellidos != None:
        texto = f"Bienvenido, { nombre } { apellidos } a mi Página de información "
...
def insertar_coche():
    ...
    cursor.execute(
        f"INSERT INTO coches VALUES(NULL, 'Lambo', 'Gallardo', 100000, 'Los Angeles')")
    cursor.connection.commit()
    return redirect('index')    
"""

@app.route('/crear_coche', methods=['GET','POST'])
def crear_coche():
...
def borrar_coche(coche_id):
        ...
        flash('Has eliminado el coche correctamente !! :( ')
